...
// CH query
  // https://collection.cooperhewitt.org/search/collection?query=&accession_number=&on_display=&has_images=1&has_no_known_copyright=1&color=red%2Corange%2Cyellow%2Cgreen%2Cblue%2Cpurple%2Cblack%2Cbrown%2Cwhite&exhibition=&location=&medium=*&period=&person=&role=&tag=&type=*&description=&justification=&title=&display_date=&year_acquired=&year_end=&year_start=gte1721&width=&height=&depth=&longestside=&shortestside=&isa=objects
...
const all = '*'
const url = `${searchBaseURL}?${method}&access_token=${CH_KEY}&year_started=${year}&color=${color}&type_id=${all}&medium_id=${all}&location=${all}&${img}&${ip}`
...
function fetchSearchData(url) {
  ...
  request(url, function(error, response, body) {
    ...
    let pageSize = 100;
    // let totalObjects = 15321;
    // let numberOfQueries = Math.ceil(totalObjects / pageSize);
...
function fetchUrl(searchAllURL){
  request(searchAllURL, function (error, response, body) {
    ...
    let objects = obj.response.rows.filter(data => {
      ...
      if(data.content.descriptiveNonRepeating.objects.type_id == null)dataComplete=false;
      if(data.content.descriptiveNonRepeating.objects.year_start == null)dataComplete=false;
      if(data.content.descriptiveNonRepeating.objects.woe_id == null)dataComplete=false;
      ...
      let filename = data.content.descriptiveNonRepeating.objects[0].split('=').pop();
      ...
      return { 
        ...
        // origin: data.woe:country,
        primaryImage: data.content.descriptiveNonRepeating.images[0].content + `&${imgSizeParam}=${imgSizeValue}`,
...
setTimeout(() => {
    fs.writeFileSync('data/testData.json', JSON.stringify(myArray), 'utf8')
}, 5000)
